...
const register = async (req, res) => {
  try {
    ...
    const confirmUrl = `${process.env.BACKEND_URL}/api/auth/confirm/${confirmationToken}`;
...
const confirmAccount = async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(400)
        .json({ error: 'Token inválido o expirado. Solicita otro registro.' });
    }
...
const login = async (req, res) => {
  try {
    ...
    if (!user.isActive) {
      return res
        .status(403)
        .json({ error: 'Cuenta no activada. Revisa tu correo.' });
    }
